...
function c100304001.initial_effect(c)
	...
	e1:SetCondition(c100304001.sprcon)
	e1:SetOperation(c100304001.sprop)
	c:RegisterEffect(e1)
	--destroy
	...
	e2:SetCondition(c100304001.descon)
	...
	e4:SetTarget(c100304001.tgtg)
	e4:SetOperation(c100304001.tgop)
	c:RegisterEffect(e4)
end
function c100304001.sprfilter(c)
	return c:IsRace(RACE_DINOSAUR) and c:IsAbleToRemoveAsCost()
end
function c100304001.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c100304001.sprfilter,tp,LOCATION_GRAVE,0,2,nil)
end
function c100304001.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c100304001.sprfilter,tp,LOCATION_GRAVE,0,2,2,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c100304001.descon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()==PHASE_MAIN1 or Duel.GetCurrentPhase()==PHASE_MAIN2
end
function c100304001.posfilter(c)
...
function c100304001.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsType,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil,TYPE_MONSTER)
		and Duel.IsExistingMatchingCard(c100304001.posfilter,tp,0,LOCATION_MZONE,1,nil) end
	local g=Duel.GetMatchingGroup(c100304001.posfilter,tp,0,LOCATION_MZONE,nil)
...
function c100304001.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g1:GetCount()>0 and Duel.Destroy(g1,REASON_EFFECT)~=0 then
		local g2=Duel.GetMatchingGroup(c100304001.posfilter,tp,0,LOCATION_MZONE,nil)
		if g2:GetCount()>0 then
			Duel.ChangePosition(g2,POS_FACEDOWN_DEFENSE)
		end
	end
end
function c100304001.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c100304001.tgop(e,tp,eg,ep,ev,re,r,rp)
