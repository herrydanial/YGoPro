...
function c800000043.initial_effect(c)
	...
	e1:SetOperation(c800000043.operation)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_MATERIAL_CHECK)
	e2:SetValue(c800000043.valcheck)
...
function c800000043.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if bit.band(flag,0x3)~=0 then
		...
		if bit.band(flag,0x1)~=0 then
			e1:SetDescription(aux.Stringid(800000043,0))
			e1:SetValue(1)
		else
			e1:SetDescription(aux.Stringid(800000043,1))
			e1:SetValue(2)
		end
	...
	if bit.band(flag,0x4)~=0 then
		...
		e4:SetValue(c800000043.efilter)
		e4:SetOwnerPlayer(tp)
		e4:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
		c:RegisterEffect(e4)
	end
end
function c800000043.efilter(e,re)
	return e:GetOwnerPlayer()~=re:GetOwnerPlayer()
end
function c800000043.lvfilter(c,fc)
	return c:IsCode(16178681) or c:CheckFusionSubstitute(fc)
end
function c800000043.imfilter(c)
	return c:IsLocation(LOCATION_MZONE) and c:IsSummonType(SUMMON_TYPE_PENDULUM)
end
function c800000043.valcheck(e,c)
	local g=c:GetMaterial()
	local flag=0
	if g:GetCount()==2 then
		local lv=0
		local lg1=g:Filter(c800000043.lvfilter,nil,c)
	...
	if g:IsExists(c800000043.imfilter,1,nil) then
		flag=flag+0x4
	end
