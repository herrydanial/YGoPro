...
function c100405026.initial_effect(c)
	--activate
	...
	e3:SetDescription(aux.Stringid(100405026,0))
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_SZONE)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetCountLimit(1)
	e3:SetTarget(c100405026.tgtg)
	e3:SetOperation(c100405026.tgop)
	c:RegisterEffect(e3)
	--return
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(100405026,1))
	e4:SetCategory(CATEGORY_TOHAND)
	e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e4:SetCode(EVENT_DESTROYED)
	e4:SetProperty(EFFECT_FLAG_DELAY)
	e4:SetCondition(c100405026.thcon)
...
function c100405026.tgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c100405026.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c100405026.filter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c100405026.tgop(e,tp,eg,ep,ev,re,r,rp)
...
function c100405026.thcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
