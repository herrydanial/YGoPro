...
function c100418003.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100418003,0))
	e1:SetCategory(CATEGORY_EQUIP)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCode(EVENT_ATTACK_ANNOUNCE)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(c100418003.eqcon)
	e1:SetTarget(c100418003.eqtg)
	e1:SetOperation(c100418003.eqop)
	c:RegisterEffect(e1)
	--negate
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100418003,1))
	...
	e3:SetDescription(aux.Stringid(100418003,2))
...
function c100418003.eqcon(e)
	return Duel.GetAttacker():IsSetCard(0x3b)
end
function c100418003.eqfilter(c,tp)
	return c:IsRace(RACE_WARRIOR) and c:CheckUniqueOnField(tp) and c:IsType(TYPE_MONSTER) and not c:IsForbidden()
end
function c100418003.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c100418003.eqfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c100418003.eqfilter,tp,LOCATION_GRAVE,0,1,nil,tp)
		and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,c100418003.eqfilter,tp,LOCATION_GRAVE,0,1,1,nil,tp)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
end
function c100418003.eqop(e,tp,eg,ep,ev,re,r,rp)
...
function c100418003.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local sg=g:Filter(c100418003.spfilter2,nil,e,tp)
