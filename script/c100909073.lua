...
function c100909073.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(100909073,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_ACTIVATE)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DELAY)
	e2:SetCost(c100909073.cost)
	e2:SetTarget(c100909073.target)
	...
	e4:SetDescription(aux.Stringid(100909073,2))
	e4:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e4:SetProperty(EFFECT_FLAG_DELAY)
	e4:SetCode(EVENT_TO_GRAVE)
	e4:SetCondition(c100909073.thcon)
	e4:SetTarget(c100909073.thtg)
	e4:SetOperation(c100909073.thop)
	c:RegisterEffect(e4)
end
...
function c100909073.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:GetHandler():IsLocation(LOCATION_SZONE)
		and eg:IsExists(c100909073.cfilter,1,nil,e,tp)
