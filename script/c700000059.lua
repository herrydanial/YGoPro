--手札抹殺
function c700000059.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_HANDES+CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c700000059.target)
	e1:SetOperation(c700000059.activate)
	c:RegisterEffect(e1)
end
function c700000059.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
...
function c700000059.activate(e,tp,eg,ep,ev,re,r,rp)
	local h1=Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)
	local h2=Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)
