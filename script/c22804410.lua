...
function c22804410.spfilter(c,tp)
	return c:IsPreviousSetCard(0xdd) and c:GetPreviousControler()==tp and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
		and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer()~=tp and Duel.GetMatchingGroupCount(c22804410.filter,c:GetControler(),LOCATION_GRAVE+LOCATION_HAND+LOCATION_REMOVED,0,nil)>2)
end
...
function c22804410.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SelectTarget(tp,Card.IsRace,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,RACE_DRAGON)
end
...
function c22804410.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local g=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_MZONE+LOCATION_SZONE+LOCATION_HAND,nil)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,g:GetCount(),0,0)
	
	--if chk==0 then return true end
	--local g=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_MZONE+LOCATION_SZONE+LOCATION_HAND+LOCATION_EXTRA,nil)
	--Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,g:GetCount(),0,0)
