...
function c80000106.initial_effect(c)
	...
	e1:SetCountLimit(1,57143342)
	e1:SetCondition(c80000106.condition)
	e1:SetTarget(c80000106.target)
	e1:SetOperation(c80000106.operation)
	c:RegisterEffect(e1)
end

function c80000106.condition(e)
	return e:GetHandler():IsReason(REASON_DESTROY)
		and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD)
end

function c80000106.filter(c,e,tp)
	return c:IsSetCard(0x9A) and not c:IsCode(80000106) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

function c80000106.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c80000106.filter(chkc,e,tp) end
