...
--Jurassic Impact
--Script by nekrozar
function c100217034.initial_effect(c)
	--Activate
	...
	e1:SetCondition(c100217034.condition)
	e1:SetTarget(c100217034.target)
	e1:SetOperation(c100217034.activate)
	c:RegisterEffect(e1)
end
function c100217034.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(Card.IsRace,tp,LOCATION_MZONE,0,2,nil,RACE_DINOSAUR) and Duel.GetLP(tp)<Duel.GetLP(1-tp)
end
function c100217034.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c100217034.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetFieldGroup(tp,LOCATION_MZONE,LOCATION_MZONE)
