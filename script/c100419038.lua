...
function c100419038.efffilter(c,g,ignore_flag)
	return c:IsFaceup() and c:IsType(TYPE_EFFECT) and c:IsSetCard(0x207)
		and c:GetSequence()<5 and g:IsContains(c) and (ignore_flag or c:GetFlagEffect(100419038)==0)
end
function c100419038.effop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local cg=c:GetColumnGroup(1,1)
	local g=Duel.GetMatchingGroup(c100419038.efffilter,tp,LOCATION_MZONE,0,nil,cg)
...
function c100419038.datg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return gc and gc:IsFaceup() and gc:IsLocation(LOCATION_SZONE)
		and not gc:IsDisabled() and c100419038.efffilter(c,gc:GetColumnGroup(1,1),true)
