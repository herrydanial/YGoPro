...
--Script by nekrozar
function c100909046.initial_effect(c)
	c:EnableReviveLimit()
	--pendulum summon
	aux.EnablePendulumAttribute(c,false)
	--synchro summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100909046,0))
	...
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetDescription(aux.Stringid(100909046,1))
	e2:SetCondition(c100909046.syncon)
	e2:SetTarget(c100909046.syntg)
	e2:SetOperation(c100909046.synop)
	...
	e3:SetCondition(c100909046.indcon)
	e3:SetOperation(c100909046.indop)
	c:RegisterEffect(e3)
	--atkdown
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(100909046,2))
	e4:SetCategory(CATEGORY_ATKCHANGE)
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e4:SetCode(EVENT_DAMAGE_STEP_END)
	e4:SetRange(LOCATION_PZONE)
	e4:SetCondition(c100909046.atkcon)
	e4:SetOperation(c100909046.atkop)
	c:RegisterEffect(e4)
	--salvage
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(100909046,3))
	e5:SetCategory(CATEGORY_TOHAND)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e5:SetCode(EVENT_SPSUMMON_SUCCESS)
	e5:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e5:SetCondition(c100909046.thcon)
	...
	e6:SetOperation(c100909046.lpop)
	c:RegisterEffect(e6)
	--pendulum
	local e7=Effect.CreateEffect(c)
	e7:SetDescription(aux.Stringid(100909046,5))
	e7:SetCategory(CATEGORY_DESTROY)
	e7:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e7:SetCode(EVENT_DESTROYED)
	e7:SetProperty(EFFECT_FLAG_DELAY)
	e7:SetCondition(c100909046.pencon)
	e7:SetTarget(c100909046.pentg)
	e7:SetOperation(c100909046.penop)
	c:RegisterEffect(e7)
end
...
function c100909046.synfilter(c,syncard,lv,g2)
...
function c100909046.syncon(e,c,tuner,mg)
	...
	local g2=nil
	if mg then
		g1=mg:Filter(c100909046.matfilter1,nil,c)
		g2=mg:Filter(c100909046.matfilter2,nil,c)
	else
		g1=Duel.GetMatchingGroup(c100909046.matfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
		g2=Duel.GetMatchingGroup(c100909046.matfilter2,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
	end
	...
	if not pe then
		return g1:IsExists(c100909046.synfilter,1,nil,c,lv,g2)
	else
		return c100909046.synfilter(pe:GetOwner(),c,lv,g2)
	end
end
function c100909046.syntg(e,tp,eg,ep,ev,re,r,rp,chk,c,tuner,mg)
	...
	if tuner then
		...
		local tuner=nil
		if not pe then
			local t1=g1:FilterSelect(tp,c100909046.synfilter,1,1,nil,c,lv,g2)
			tuner=t1:GetFirst()
		else
			tuner=pe:GetOwner()
...
function c100909046.synop(e,tp,eg,ep,ev,re,r,rp,c,tuner,mg)
	local g=e:GetLabelObject()
	c:SetMaterial(g)
	Duel.SendtoGrave(g,REASON_MATERIAL+REASON_SYNCHRO)
	g:DeleteGroup()
end
function c100909046.indcon(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	return a:IsType(TYPE_PENDULUM) and a:IsControler(tp)
end
function c100909046.indop(e,tp,eg,ep,ev,re,r,rp)
...
function c100909046.thop(e,tp,eg,ep,ev,re,r,rp)
...
function c100909046.pencon(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(r,REASON_EFFECT+REASON_BATTLE)~=0 and e:GetHandler():IsPreviousLocation(LOCATION_MZONE)
end
function c100909046.pentg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLocation(tp,LOCATION_SZONE,6) or Duel.CheckLocation(tp,LOCATION_SZONE,7) end
end
function c100909046.penop(e,tp,eg,ep,ev,re,r,rp)
