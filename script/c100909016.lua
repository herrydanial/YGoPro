...
function c100909016.initial_effect(c)
	...
	e2:SetCondition(c100909016.thcon)
	e2:SetTarget(c100909016.thtg)
	e2:SetOperation(c100909016.thop)
	c:RegisterEffect(e2)
end
...
function c100909016.filter(c)
	return c:IsRace(RACE_FIEND) and c:IsAttribute(ATTRIBUTE_FIRE) and not c:IsCode(100909016) and c:IsAbleToHand()
end
function c100909016.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c100909016.filter(chkc) end
...
function c100909016.thcon(e,tp,eg,ep,ev,re,r,rp)
	local ec=eg:GetFirst()
	return ec:GetSummonType()==SUMMON_TYPE_SYNCHRO and ec:GetSummonPlayer()==tp and aux.exccon(e)
end
function c100909016.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand() end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,e:GetHandler(),1,0,0)
end
function c100909016.thop(e,tp,eg,ep,ev,re,r,rp)
