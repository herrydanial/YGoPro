...
function c101002023.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(101002023,1))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_BATTLE_DAMAGE)
	e3:SetCountLimit(1,101002023+100)
	e3:SetCondition(c101002023.spcon)
	e3:SetTarget(c101002023.sptg)
...
function c101002023.spcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
...
function c101002023.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c101002023.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c101002023.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tc=Duel.SelectMatchingCard(tp,c101002023.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp):GetFirst()
	if tc and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)~=0 then
		tc:RegisterFlagEffect(101002023,RESET_EVENT+0x1fe0000,0,1)
		...
		e2:SetCondition(c101002023.descon)
		e2:SetOperation(c101002023.desop)
		Duel.RegisterEffect(e2,tp)
	end
end
function c101002023.descon(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
	if tc:GetFlagEffect(101002023)~=0 then
...
function c101002023.desop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject()
