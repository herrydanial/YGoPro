...
function c100000400.filter(c,e,tp)
	...
	return c:IsFaceup() and c:IsType(TYPE_XYZ) 
	 and Duel.IsExistingMatchingCard(c100000400.xyzfilter,tp,LOCATION_EXTRA,0,1,nil,rank,e,tp)
end
function c100000400.xyzfilter(c,rank,e,tp)
	return c:GetRank()==rank+1 and (c:IsSetCard(0x86) or c:IsSetCard(0x1048))
...
function c100000400.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		sg:AddCard(tc)
		local xyzg=Duel.GetMatchingGroup(c100000400.xyzfilter,tp,LOCATION_EXTRA,0,nil,rank,e,tp)
