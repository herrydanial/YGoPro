...
function c69.initial_effect(c)
	...
	e1:SetCondition(c69.condition)
	e1:SetOperation(c69.operation)
	c:RegisterEffect(e1)
	--Immune
	...
	e4:SetCondition(c69.discon)
	e4:SetTarget(c69.distg)
	e4:SetOperation(c69.disop)
	c:RegisterEffect(e4)
	--Negate Summon
	...
	e5:SetCondition(c69.condition2)
	...
	e17:SetDescription(aux.Stringid(69,0))
	...
	e17:SetCountLimit(5)
	e17:SetTarget(c69.tktg)
	e17:SetOperation(c69.tkop)
	c:RegisterEffect(e17)
	--Special Summon opponent's turn
	local e18=Effect.CreateEffect(c)
	e18:SetDescription(aux.Stringid(69,0))
	e18:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e18:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e18:SetCode(EVENT_PHASE+PHASE_DRAW)
	e18:SetRange(LOCATION_EXTRA)
	e18:SetCountLimit(1)
	e18:SetCondition(c69.accon)
	e18:SetTarget(c69.actg)
	e18:SetOperation(c69.acop)
...
function c69.accon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)==0
end
function c69.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end	
end
function c69.acop(e,tp,eg,ep,ev,re,r,rp)
...
function c69.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,420,0,0x4011,0,9999999,12,RACE_BEAST,ATTRIBUTE_DIVINE) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,5,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,5,0,0)
end
function c69.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,420,0,0x4011,0,9999999,12,RACE_BEAST,ATTRIBUTE_DIVINE) then return end
	local token=Duel.CreateToken(tp,420)
	Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)
end
function c69.efilter(e,te)
	return te:GetOwner()~=e:GetOwner()
end
function c69.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c69.operation(e,tp,eg,ep,ev,re,r,rp,chk)
	local a=Duel.GetAttacker()
	local d=Duel.GetAttackTarget()
...
function c69.discon(e,tp,eg,ep,ev,re,r,rp)
	return re:GetHandler()~=e:GetHandler() and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED)
		and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
end
function c69.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
...
function c69.disop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.NegateActivation(ev) then return end
	if re:GetHandler():IsRelateToEffect(re) then 
		Duel.Destroy(eg,REASON_EFFECT)
	end
end
function c69.condition2(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentChain()==0
end
function c69.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
